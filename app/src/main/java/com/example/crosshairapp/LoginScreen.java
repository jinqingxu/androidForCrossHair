...
public class LoginScreen extends Activity {
    ...
    int mode=0; // 0 represents crosshair and 1 represents real experiment
    ...
    public void  createTimestampFile(){
        ...
        if(Environment.MEDIA_MOUNTED.equals(state)){    // MEDIA_MOUNTED: constant, storage state if the media is present and mounted at its mount point with read/write access.
            ...
            try {
                ...
                String header="FirstTouchDownTimeStamp"+","+"FinalLiftUpTimeStamp"+'\n'; // the header for the timestamp file
                out.write(header);
                out.close();
    ...
    /*public void createCrossHairTimeStamp(){

        FileOutputStream file = openFileOutput( "crossHair.txt", Context.MODE_PRIVATE | Context.MODE_WORLD_READABLE);
    ...
    public void createCsvFile(String filename,String header){
        String state = Environment.getExternalStorageState(); //Returns the current state of the primary shared/external storage media.
